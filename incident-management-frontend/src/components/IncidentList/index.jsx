...
import { Table, Button, Avatar, Popconfirm, Modal, Select, Pagination } from "antd";
...
export default function IncidentList(props) {
    ...
    useEffect(() => {
        HTTP.get("/api/list")
             .then((rsp) => {
                 setDataSource(rsp.content);
                 setTotal(rsp.totalElements)
             })
             .catch((error) => {
                 console.log(error)
             })
    ...
    return (
        <div className="incident-list">
            ...
            <Pagination current={page} pageSize={size} total={total} onChange={onChange} showSizeChanger onShowSizeChange={onShowSizeChange} />
        </div>
    )
}
